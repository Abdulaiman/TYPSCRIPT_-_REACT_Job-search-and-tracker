...
const InterviewScheduled: React.FC = (): JSX.Element => {
  const [jobs, setJobs] = useState<Istate["cards"]>([]);
  const myJob: Boolean = true;
  const jobText: String = "Interview Scheduled Jobs";
  const additionalQuery: String = "/get-interview-scheduled-jobs";
  const token = localStorage.getItem("token");

  useEffect(() => {
    const fetchData = async () => {
      const data = await axios.get(
        `${DOMAIN.URL}/api/v1/save-job/get-interview-scheduled-jobs`,
        {
